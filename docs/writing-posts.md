...
- `is_published`: set to `true` when ready.
- `published_at`: publish timestamp. Use `now()` or an ISO time.

Readers can comment directly on published posts. Comments are stored in `post_comments` and do not require moderation.
